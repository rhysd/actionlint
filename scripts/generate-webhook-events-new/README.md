...
Unlike [`../generate-webhook-events`](../generate-webhook-events), this script does not parse the markdown source. It parses the rendered HTML document instead.

## Usage

```
generate-webhook-events-new [[srcfile] dstfile]
```

Generate `all_webhooks.go` file:

```sh
go run ./scripts/generate-webhook-events-new ./all_webhooks.go
```

When the HTML file is in local:

```sh
go run ./scripts/generate-webhook-events-new ./input.html ./all_webhooks.go
```

For debugging, specifying `-` to `dstfile` outputs the generated source to stdout:

```sh
go run ./scripts/generate-webhook-events-new ./input.html -
```
