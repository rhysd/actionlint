...
func (rule *RuleBranding) VisitActionPre(n Action) error {
	b := n.Common().Branding
	if b == nil { // Branding is optional
		return nil
	}

	valid_colors := []string{"white", "yellow", "blue", "green", "orange", "red", "purple", "gray-dark"}
	if b.Color != nil && !contains(valid_colors, b.Color.Value) {
		rule.Errorf(b.Color.Pos, "invalid color %q. valid colors are %v", b.Color.Value, valid_colors)
	}
	...
	if b.Icon != nil && !contains(valid_icons, b.Icon.Value) {
		rule.Errorf(b.Icon.Pos, "invalid icon %q", b.Icon.Value)
	}
	return nil
}
